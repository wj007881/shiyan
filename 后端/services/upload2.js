...
let upload2 = async   function (ctx, next) {
  let result;
 ...
 // if (!util.isArray(files))
  ...
  for (let key in files) {
    ...
    filePaths.push(filePath)
                ...
                Techer.findOne({
          "_id":id,
          "shiyanbaogao.fabuname":fabuname
        },null,function(err,documents){
          if(err){}else{
        documents.shiyanbaogao[0].baogaocount.push({baogao: file.name});
        documents.save();
          }
        })
